  ...
  return (
    <div className="p-10 mt-18">
      <div className="mb-7 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <h2 className="m-0 text-2xl font-semibold t-text-heading">Company Management</h2>
          <p className="mt-1 text-sm t-text-subtle">Manage tenants, subscriptions, and admin access.</p>
        </div>
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
          ...
          <select value={status} onChange={(event) => setStatus(event.target.value)} className="border t-border-in bg-[var(--bg-input)] px-3 py-3 text-sm">
            <option value="">All Status</option>
            <option value="active">Active</option>
            <option value="inactive">Inactive</option>
            <option value="suspended">Suspended</option>
          </select>
          <select value={plan} onChange={(event) => setPlan(event.target.value)} className="border t-border-in bg-[var(--bg-input)] px-3 py-3 text-sm">
          ...
          <select value={`${sort}:${order}`} onChange={(event) => {
            ...
            setSort(nextSort);
            setOrder(nextOrder);
          }} className="border t-border-in bg-[var(--bg-input)] px-3 py-3 text-sm">
      ...
      <DataTable headers={['Logo', 'Company Name', 'Company Admin', 'Email', 'Plan', 'Employees', 'Database Name', 'Status', 'Created Date', 'Actions']}>
          ...
          <tr key={company.id} className="border-t t-divider">
            ...
            <td className="px-5 py-4">
              <div className="flex items-center gap-2">
                ...
                <button title="Login as Admin" type="button" onClick={() => handleImpersonate(company)} className="text-[var(--text-secondary)] hover:text-blue-700">
                  <LogIn size={18} />
                </button>
        ...
        {!companies.length && (
          <tr>
            <td colSpan="10" className="px-5 py-8 text-center text-sm t-text-subtle">
      ...
      <div className="mt-5 flex items-center justify-between text-sm t-text-muted">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="border t-border-in bg-[var(--bg-input)] px-3 py-2 disabled:opacity-50"
          >
            Previous
          </button>
