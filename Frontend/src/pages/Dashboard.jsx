...
const cards = (stats) => [
  ...
  },
  {
    ...
    bg: 'bg-neutral-800/80 border border-neutral-700',
    bar: 'bg-neutral-400',
    to: ROUTES.MEETINGS,
  },
  {
  ...
  return (
    <div className="p-6 pt-8 sm:p-8 lg:p-10">
      <div className="mb-8">
        ...
        <h2 className="m-0 text-3xl font-semibold tracking-tight text-neutral-50">Dashboard</h2>
        <p className="mt-2 text-sm text-neutral-400">A clear view of your company operations.</p>
      </div>
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-2">
        {cards(stats).map((c) => (
          <button
            ...
            className="group relative cursor-pointer overflow-hidden rounded-2xl border border-neutral-800/80 bg-neutral-900/40 px-6 pb-6 pt-6 text-left shadow-[0_0_24px_rgba(239,68,68,0.06)] backdrop-blur-md transition duration-200 hover:-translate-y-1 hover:scale-[1.01] hover:border-red-500/30 hover:shadow-[0_0_28px_rgba(239,68,68,0.15)] focus:outline-none focus:ring-2 focus:ring-red-500/40"
          >
            <div className={`mb-4 flex h-12 w-12 items-center justify-center rounded-xl ${c.bg}`}>
              <c.icon size={24} className={c.color} />
            </div>
            <div className="text-4xl font-semibold leading-none tracking-tight text-neutral-50">{c.value}</div>
            <div className="mt-3 font-mono text-xs font-medium uppercase tracking-[0.12em] text-neutral-500">
              {c.label}
            </div>
